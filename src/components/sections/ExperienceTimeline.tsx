		...
		<div className="max-w-6xl mx-auto px-4 sm:px-6">
			...
			<div className="mb-16">
				...
				<p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-xl">
					1 Year and 3 Months of full-time and internship experience delivering
					high-concurrency systems, core banking platforms, and AI optimization.
				</p>
			</div>
			...
			<div className="space-y-12 relative before:absolute before:inset-0 before:left-4 md:before:left-1/2 before:-ml-px before:w-0.5 before:bg-gradient-to-b before:from-cyan-500 before:via-blue-500 before:to-transparent">
				{PORTFOLIO_DATA.experience.map((exp: Experience, idx: number) => (
					...
					>
						{/* Timeline Icon Node */}
						<div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-900 border-2 border-cyan-500 flex items-center justify-center text-cyan-500 dark:text-cyan-400 shadow-lg shadow-cyan-500/30 group-hover:scale-110 transition-transform z-10">
						...
						<div className="ml-12 md:ml-0 md:w-full grid grid-cols-1 md:grid-cols-2 gap-8">
							...
							<div className="md:pr-12 md:text-right">
								<div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-mono text-cyan-600 dark:text-cyan-400 mb-2">
									<Calendar className="w-3.5 h-3.5" />
									{exp.period}
								</div>
								<h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center md:justify-end gap-2">
									<Building2 className="w-5 h-5 text-cyan-500 dark:text-cyan-400 hidden md:inline" />
									{exp.company}
								</h3>
								...
								{exp.clients.length > 0 && (
									<div className="inline-flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400 font-mono bg-amber-500/10 px-2.5 py-1 rounded-md border border-amber-500/20 mt-1">
										<Award className="w-3.5 h-3.5" /> Clients:{" "}
										{exp.clients.join(", ")}
									</div>
								)}
							</div>
							...
							<div className="glass-card p-6 rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-cyan-500/40 transition-colors">
								...
								<div className="flex flex-wrap gap-1.5 pt-4 border-t border-slate-200/60 dark:border-slate-800/60">
									{exp.skills.map((skill: string) => (
										<span
											key={skill}
											className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-900 text-[11px] font-mono text-cyan-600 dark:text-cyan-300 border border-slate-200 dark:border-slate-800"
										>
											{skill}
										</span>
									))}
