			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
				{PORTFOLIO_DATA.projects.map((project: Project, idx: number) => (
					<motion.div
						...
						className="glass-card p-6 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between group hover:border-cyan-500/50 transition-all"
					>
						<div>
							{/* Category & Date Header */}
							<div className="flex items-center justify-between mb-4">
								<span className="text-xs font-mono text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 px-2.5 py-1 rounded-md border border-cyan-500/20">
									{project.category}
								</span>
								{project.date && (
							...
							<h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2 group-hover:text-cyan-500 dark:group-hover:text-cyan-400 transition-colors">
								{project.title}
							</h3>
							...
							<div className="mb-6 space-y-2">
								<h4 className="text-[11px] font-mono text-slate-500 dark:text-slate-400 uppercase">
									Key Features:
								</h4>
						...
						<div className="pt-4 border-t border-slate-200/60 dark:border-slate-800/60">
							<div className="flex flex-wrap gap-1.5 mb-4">
								{project.techStack.map((tech: string) => (
									<span
										key={tech}
										className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 text-[11px] font-mono text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800"
									>
										{tech}
									</span>
								))}
							...
							<div className="flex items-center justify-end gap-3 pt-2">
								{project.githubUrl && (
									<a
										...
										className="text-xs text-slate-500 dark:text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 flex items-center gap-1 font-mono"
									>
