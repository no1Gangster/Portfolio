...
const MetricsSpotlight: React.FC = (): React.ReactElement => (
	<section className="py-12 bg-slate-100/80 dark:bg-slate-900/50 border-y border-slate-200/60 dark:border-slate-800/60">
				...
				{PORTFOLIO_DATA.metrics.map((metric: Metric, idx: number) => (
					...
					>
						<div className="flex items-center mb-4 gap-4">
							<div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 group-hover:border-cyan-500/40 transition-colors">
								{iconMap[metric.iconName] || (
									<Zap className="w-6 h-6 text-cyan-400" />
								)}
							</div>
							<span className="text-3xl font-extrabold font-mono text-slate-800 dark:text-slate-100 group-hover:text-cyan-500 dark:group-hover:text-cyan-400 transition-colors">
								{metric.value}
							</span>
						</div>
